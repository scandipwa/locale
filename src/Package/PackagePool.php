...
namespace ScandiPWA\Locale\Package;
...
use Magento\Deploy\Package\PackagePool as MagentoPackagePool;

/**
 * Deployment Packages Pool class
 */
class PackagePool extends MagentoPackagePool
{
    /**
     * @var Collector
     */
    ...
    public function getPackagesForDeployment(array $options)
    {
        ...
        // Check for PWA theme
        if ($this->includedThemeIsPwa($options)) {
            $this->packages = $this->getPwaPackages($options);
            return $this->packages;
        }
    ...
    private function filterArrayByKeys(
        array $array,
        array $keysToFind,
        callable $valueCheck = null
    ): array {
        $quotedKeysToFind = array_map(
            function ($key) {
        ...
        return array_filter(
            ...
            function ($key) use ($quotedKeysToFind, $valueCheck, $array) {
                ...
                return $valueCheck($array[$key]);
            },
            ARRAY_FILTER_USE_KEY
        );
    }

    private function getPwaPackages($options)
    {
        $includedThemes = $this->getOption(Options::THEME, $options);
        return $this->filterArrayByKeys($this->packages, $includedThemes);
    }

    private function includedThemeIsPwa($options)
    {
        $includedThemes = $this->getOption(Options::THEME, $options);
        $includedPwaThemes = $this->filterArrayByKeys(
            $this->themes,
            $includedThemes,
            function ($theme) {
                /** @var $theme ThemeInterface */
                return $theme->getType() === 4;
            }
        );

        return count($includedPwaThemes);
    }
